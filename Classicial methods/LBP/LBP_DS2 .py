'''
LBP facial recognition of the second dataset (Georgia tech face)
'''
...
def calculate_lbp_descriptor(gray_image, raduis, n_points):
    # calculating the lbp image
    lbp = local_binary_pattern(gray_image, n_points, raduis, 'default')
    imgLBP = np.uint8(lbp)

    # calculating the lbp histogram
    descriptor = cv.calcHist([imgLBP], [0], None, [256], [0, 256])  # calculating histogram # calculating histogram
...
def lbp_recognition(train_dataset_path, test_dataset_path, knn, tatal_tr_imgs, total_tst_imgs, train_img_nbr_class, tst_img_nbr_class, raduis, n_points):
    ...
    for folder in test_dataset_files:
        ...
        for file in folder_files:
            ...
            # print('possible classes', img_possible_classes)
...
lbp_recognition(train_dataset_path=train_path, test_dataset_path=test_path, knn=1, tatal_tr_imgs=600, total_tst_imgs=150, train_img_nbr_class=12,
                                 tst_img_nbr_class=3, raduis=1, n_points=8)