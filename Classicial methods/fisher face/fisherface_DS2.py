...
def eigen_face(train_dataset_path, test_dataset_path, knn, total_tr_imgs, total_tst_imgs, train_img_nbr_class, tst_img_nbr_class):
    # for train -------------------------------------------------------------------------------
    ...
    for folder in train_dataset_files:
        ...
        for file in folder_files:
            # read image
            img = np.array(cv.imread(train_dataset_path + folder + '/' + file, cv.IMREAD_GRAYSCALE)) # read as grayscale
            resized_img = cv.resize(img, (150, 200))
    ...
    lda = LDA()
    train_principalComponents = lda.fit_transform(train_st_sub_image_vect, train_classes)
    ...
    for folder in test_dataset_files:
        ...
        for file in folder_files:
            ...
            test_principalComponents = lda.transform(reshaped_test_vect)
    ...
    if accuracy_new[0][0] == 0:
        print(f'accuracy (k = {knn}) = ', 1 - (accuracy_new[0][1] / total_tst_imgs))
    else:
        print(f'accuracy (k = {knn}) = ', accuracy_new[0][1] / total_tst_imgs)
...
train_path = f'../../datasets/dataset2/fold {fold}/orig_images/training/'
test_path = f'../../datasets/dataset2/fold {fold}/orig_images/testing/'

eigen_face(train_dataset_path=train_path, test_dataset_path=test_path, knn=1, total_tr_imgs=600, 
                    total_tst_imgs=150, train_img_nbr_class=12, tst_img_nbr_class=3)
